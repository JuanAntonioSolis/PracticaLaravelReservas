...
<main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
        <div class="text-[13px] leading-[20px] flex-1 p-6 pb-12 lg:p-20 bg-white dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-bl-lg rounded-br-lg lg:rounded-tl-lg lg:rounded-br-none">
            <h1 class="mb-4 font-medium">RESERVAS:</h1>


            <form method="POST" action="{{route('reservas.filtrar')}}" >
                @csrf
                <div class="row-cols-2 d-flex mb-5 ">
                ...
                <button type="submit" class="w-full bg-black text-white py-2 rounded-lg hover:bg-gray-800 transition mb-4">
                    Filtrar
                </button>
            </form>
                ...
                @foreach($reservas as $reserva)
                    <li class="px-4 py-3 flex items-center justify-between">
                        ...
                        <div class="text-sm text-gray-500">
                            Sala {{$reserva->sala->id}} Personas {{$reserva->numPersonas}}
                        </div>
                        @php
                        ...
                        <div class="text-sm text-red-400 hover:underline d-flex gap-2">
                            <a href="{{ route('reservas.confirmar', [ 'reserva' => $reserva->id ]) }}">
